...
Help a user configure Linear for ORCA Framework work: states, labels, guidance, permissions, agent access, smoke-test issue, and opt-out rules.

## When To Use

Use when a team is adopting ORCA Framework with Linear, creating a new Linear project, adding agents to an existing Linear workspace, or diagnosing whether Linear is ready for ORCA Framework.
...
- Trigger: new ORCA Framework adoption, new project setup, `orca-framework` install, or `needs-linear-setup`
- Human approval: required before changing workspace-wide states, labels, permissions, or guidance

## Opt-Out Context

If the user opts out of Linear, use this command to choose and document the alternative system of record. The output should map ORCA Framework gates to that system.

## Workflow

1. Confirm whether the user wants Linear-first mode or opt-out mode.
2. Identify setup scope: workspace, team, or project.
3. Map existing states to ORCA Framework gates or recommend missing gates.
4. Map existing labels to ORCA Framework routing and risk labels or recommend additions.
...
- If agents cannot post comments, block ORCA Framework Linear-first rollout until comment access is fixed.
