...
Create the primary implementation contract for a project, feature, or fix.
...
4. Use `hvn-spec`.
5. Define problem, objective, users or usage context, v1 scope, later phase scope, out-of-scope work, constraints, assumptions, edge cases, behavior, data or API implications, likely area of change, acceptance criteria, verification criteria, and risks.
...
9. Use `templates/requirements-split.md` when scope contains current and future work.
10. Run or recommend `hvn-spec-check` before planning.
11. Ask for approval when the spec changes project direction.
12. Post or attach the spec to the work item and update run memory with approved scope, assumptions, open questions, and spec status.

## Interactive Question Flow

- Uses interactive flow: conditional
- Default round size: one missing-spec question
- Collects: behavior, actor, v1/later/out-of-scope split, acceptance criteria, verification criteria, constraints, risky assumptions
...
- If the problem is vague, return to onboarding or clarification before writing solution details.

## Related Commands And Skills

- Commands: `hvn-question-flow`, `hvn-memory-init`, `hvn-memory-read`, `hvn-memory-update`, `hvn-spec-check`, `hvn-linear-plan-comment`, `hvn-plan`, `hvn-research`
