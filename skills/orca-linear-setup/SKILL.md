...
description: Guide users through setting up Linear for ORCA Framework, including states, labels, agent guidance, permissions, smoke tests, and opt-out mapping.
---

# ORCA Framework Linear Setup

## What This Skill Is

A setup workflow for making Linear usable as ORCA Framework's default coordination layer.

## Trigger

Use when a user asks to set up Linear, adopt ORCA Framework in a Linear workspace, configure agent guidance, validate Linear readiness, or define an opt-out record.
...
3. Inventory available states and map them to ORCA Framework gates.
...
10. If opt-out, map ORCA Framework issue, project, state, comment, and artifact concepts to the chosen record.
...
The setup should let a new agent pick up an issue, know which ORCA Framework gate it is in, post the right comment, and avoid unsafe transitions.
