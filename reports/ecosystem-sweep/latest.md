...
- `Adopt now` / `workflow`: Codex `0.136.0` (June 1, 2026) adds archive/unarchive commands, richer app-server MCP status, `codex app-server --stdio`, remote `CODEX_API_KEY` registration for approved OpenAI hosts, and more command-safety hardening. ORCA-HVN's Codex host guidance now trails the real lifecycle surface.
