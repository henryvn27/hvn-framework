...
ORCA Framework command mapping separates the logical framework command from host-specific slash commands.

## ORCA Framework Goal Commands
...
## ORCA Framework Explain Command
...
| ORCA Framework action | Codex CLI | Claude Code | Fallback |
| --- | --- | --- | --- |
| Create goal | `/goal <objective>` | `/goal <condition>` when hooks and workspace trust permit it | create goal contract and run normal ORCA Framework workflow |
...
The best ORCA Framework commands to run in a side session when the host supports it are:
