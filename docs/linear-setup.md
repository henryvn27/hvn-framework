...
HVN helps teams set up Linear as the default coordination layer for agentic software work. The framework does not handle OAuth, create credentials, or assume access to a workspace. It gives users a practical setup sequence, state model, labels, guidance, and a smoke test.

## Setup Outcome

After setup, a team should have:

- A Linear workspace or team selected for HVN work.
...
HVN expects gates, not exact names. Configure or map states equivalent to:
...
Keep guidance short enough that agents can follow it. Link to HVN docs for deeper policy.
...
Decide separately whether the agent can update state, labels, assignee, or project. HVN works well when agents can recommend transitions even if humans perform them.

## Step 6: Create A Smoke-Test Issue

Create a low-risk issue:

Title: `HVN setup smoke test`

Description:

```text
Use HVN to confirm this Linear project is ready for agent coordination.

Expected result:
- Agent reads this issue.
- Agent identifies the current state and labels.
- Agent posts a short HVN setup check comment.
- Agent recommends the next gate without changing state unless allowed.
```

Labels: `needs-triage`, `docs`

Run `hvn-linear-setup` or ask the agent to perform an HVN setup check.
...
- States exist but do not represent HVN gates.
