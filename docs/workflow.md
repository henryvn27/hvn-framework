...
9. `hvn-review`: inspect behavior and maintainability.
10. `hvn-design`: review UX, accessibility, responsive behavior, and product clarity.
11. `hvn-test-blind`: run first-look QA with minimal context.
12. `hvn-test-briefed`: retest with a bounded packet.
13. `hvn-test-regression`: verify fixes and adjacent flows.
14. `hvn-security` or `hvn-security-check`: inspect security-relevant surfaces and untrusted inputs.
15. `hvn-eval`: judge trajectory quality when validating HVN workflow behavior.
